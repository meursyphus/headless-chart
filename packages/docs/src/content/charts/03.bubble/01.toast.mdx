...
files:
  App.js: |
    ...
    class AnimatedXAxis extends StatefulWidget {
      ...
      createState() {
        return new AnimatedXAxisState();
      }
    }
    
    class AnimatedXAxisState extends State {
      initState(context) {
    ...
    class AnimatedYAxis extends StatefulWidget {
      constructor({ line, labels, tick }) {
        super();
        this.line = line;
        this.labels = labels;
      ...
      createState() {
        return new AnimatedYAxisState();
      }
    }
    
    class AnimatedYAxisState extends State {
      initState(context) {
        this.animationController = new AnimationController({ duration: 300 });
        this.animationController.addListener(() => {
          this.setState();
