...
export function YAxis({
  labels,
  tick,
}: Parameters<BarChartCustom["yAxis"]>[0]) {
  return Container({
    height: Infinity,
    child: Row({
      mainAxisSize: MainAxisSize.min,
...
function Series(
  ...[{ barGroups }, { scale }]: Parameters<BarChartCustom["series"]>
) {
...
function Bar(
  ...[{ label, legend, value }, { data }]: Parameters<BarChartCustom["bar"]>
) {
    ...
    child: Stack({
      ...
      children: [
        CustomPaint({
          ...
          painter: {
            svg: {
              createDefaultSvgEl(context: SvgPaintContext) {
                return {
...
function BarGroup(
  ...[{ bars, values }, { scale }]: Parameters<BarChartCustom["barGroup"]>
) {
